...
Note: This note is adopted from [Introduction to R](https://book.stat420.org/introduction-to-r.html)
...
The dot product of vectors a=\[a1,a2,⋯an\]a=\[a1,a2,⋯an\] and b=\[b1,b2,⋯bn\]b=\[b1,b2,⋯bn\] is defined to be

a⋅b=n∑i=1aibi=a1b1+a2b2+⋯anbn.a⋅b=∑i=1naibi=a1b1+a2b2+⋯anbn.
...
```         
##      [,1] [,2] [,3] ## [1,]    2    2    2 ## [2,]    4    4    4 ## [3,]    6    6    6
```

The `%o%` operator is used to calculate the **outer product** of the two vectors.

When vectors are coerced to become matrices, they are column vectors. So a vector of length nn becomes an n×1n×1 matrix after coercion.
...
At face value this is a 3×13×1 matrix, multiplied by a 3×13×1 matrix. However, when `b_vec` is automatically coerced to be a matrix, `R` decided to make it a "row vector," a 1×31×3 matrix, so that the multiplication has conformable dimensions.
...
These functions could be very useful later. When used with matrices XX and YY as arguments, it calculates
...
```         
C_mat = matrix(c(1, 2, 3, 4, 5, 6), 2, 3) D_mat = matrix(c(2, 2, 2, 2, 2, 2), 2, 3)
```
...
```         
##      [,1] [,2] [,3] ## [1,]    6    6    6 ## [2,]   14   14   14 ## [3,]   22   22   22
```
...
```         
## $e ##      [,1] [,2] [,3] [,4] [,5] ## [1,]    1    0    0    0    0 ## [2,]    0    1    0    0    0 ## [3,]    0    0    1    0    0 ## [4,]    0    0    0    1    0 ## [5,]    0    0    0    0    1 ##  ## $a ## [1] 1 2 3 4
```
...
```         
## $e ##      [,1] [,2] [,3] [,4] [,5] ## [1,]    1    0    0    0    0 ## [2,]    0    1    0    0    0 ## [3,]    0    0    1    0    0 ## [4,]    0    0    0    1    0 ## [5,]    0    0    0    0    1
```
...
```         
##    x       y     z ## 1  1   Hello  TRUE ## 2  3   Hello FALSE ## 3  5   Hello  TRUE ## 4  7   Hello FALSE ## 5  9   Hello  TRUE ## 6  1   Hello FALSE ## 7  3   Hello  TRUE ## 8  5   Hello FALSE ## 9  7   Hello  TRUE ## 10 9 Goodbye FALSE
```
...
```         
## 'data.frame':    10 obs. of  3 variables: ##  $ x: num  1 3 5 7 9 1 3 5 7 9 ##  $ y: chr  "Hello" "Hello" "Hello" "Hello" ... ##  $ z: logi  TRUE FALSE TRUE FALSE TRUE FALSE ...
```
...
```         
library(readr) example_data_from_csv = read_csv("data/example-data.csv")
```
...
```         
## # A tibble: 10 × 3 ##        x y       z     ##    <dbl> <chr>   <lgl> ##  1     1 Hello   TRUE  ##  2     3 Hello   FALSE ##  3     5 Hello   TRUE  ##  4     7 Hello   FALSE ##  5     9 Hello   TRUE  ##  6     1 Hello   FALSE ##  7     3 Hello   TRUE  ##  8     5 Hello   FALSE ##  9     7 Hello   TRUE  ## 10     9 Goodbye FALSE
##   [1] 29 29 31 30 26 26 27 26 25 28 27 25 25 25 25 24 25 23 20 15 20 17 17 26 23 ##  [26] 26 25 24 19 14 15 17 27 30 26 29 26 24 24 22 22 24 24 17 22 21 23 23 19 18 ##  [51] 17 17 19 19 12 17 15 17 17 12 17 16 18 15 16 12 17 17 16 12 15 16 17 15 17 ##  [76] 17 18 17 19 17 19 19 17 17 17 16 16 17 15 17 26 25 26 24 21 22 23 22 20 33 ## [101] 32 32 29 32 34 36 36 29 26 27 30 31 26 26 28 26 29 28 27 24 24 24 22 19 20 ## [126] 17 12 19 18 14 15 18 18 15 17 16 18 17 19 19 17 29 27 31 32 27 26 26 25 25 ## [151] 17 17 20 18 26 26 27 28 25 25 24 27 25 26 23 26 26 26 26 25 27 25 27 20 20 ## [176] 19 17 20 17 29 27 31 31 26 26 28 27 29 31 31 26 26 27 30 33 35 37 35 15 18 ## [201] 20 20 22 17 19 18 20 29 26 29 29 24 44 29 26 29 29 29 29 23 24 44 41 29 26 ## [226] 28 29 29 29 28 29 26 26 26
```
...
```         
## # A tibble: 6 × 3 ##   manufacturer model       year ##   <chr>        <chr>      <int> ## 1 honda        civic       2008 ## 2 honda        civic       2008 ## 3 toyota       corolla     2008 ## 4 volkswagen   jetta       1999 ## 5 volkswagen   new beetle  1999 ## 6 volkswagen   new beetle  1999
```
...
```         
library(dplyr) mpg %>% filter(hwy > 35) %>% select(manufacturer, model, year)
```
...
```         
if (...) {   some R code } else {   more R code }
```

For example,

```         
x = 1 y = 3 if (x > y) {   z = x * y   print("x is larger than y") } else {   z = x + 5 * y   print("x is less than or equal to y") }
```
...
```         
fib = c(1, 1, 2, 3, 5, 8, 13, 21) ifelse(fib > 6, "Foo", "Bar")
```
...
```         
x = 11:15 for (i in 1:5) {   x[i] = x[i] * 2 }  x
```
...
```         
standardize = function(x) {   m = mean(x)   std = sd(x)   result = (x - m) / std   result }
```
...
```         
##  [1]  4.3739701  7.8123334  0.8049539  9.0578998  4.6352177 -7.5363113 ##  [7] -2.3655660 -7.7944228  6.2973004  4.0238428
```
...
```         
##  [1]  0.4058923  0.9771480 -0.1870703  1.1840886  0.4492965 -1.5729027 ##  [7] -0.7138260 -1.6157858  0.7254378  0.3477216
```
...
```         
power_of_num = function(num, power = 2) {   num ^ power }
```
...
By default, it will calculate the unbiased estimate of σ2σ2, which we will call s2s2.

s2=1n−1n∑i=1(x−¯x)2s2=1n−1∑i=1n(x−x¯)2

It will also have the ability to return the biased estimate (based on maximum likelihood) which we will call ^σ2σ^2.

^σ2=1nn∑i=1(x−¯x)2σ^2=1n∑i=1n(x−x¯)2

```         
get_var = function(x, biased = FALSE) {   n = length(x) - 1 * !biased   (1 / n) * sum((x - mean(x)) ^ 2) }
```
...
We see the function is working as expected, and when returning the unbiased estimate it matches `R`'s built-in function `var()`. Finally, let's examine the biased estimate of σ2σ2.
